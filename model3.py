...
def mainProblem_iterate_min(model1,uout,uin):
  #打印输入的值
  print(uout)
  print(uin)
  '''
...
#约束：
  ...
  print(f"pwf:{pw_f}")
  #约束：
  model1.addConstr(gp.quicksum(u_w_init[0,t] for t in range(T))<=F)
  model1.addConstr(gp.quicksum(u_v_init[0,t] for t in range(T))<=F)
  model1.addConstr(p_w == pw_f-u_w_init*d_p_w_max*pw_f)
  model1.addConstr(p_v == pv_f-u_v_init*d_p_v_max*pv_f)
  #成本:主要是弃风光惩罚
