import gurobipy as gp
import numpy as np
from gurobipy import GRB

class mainProblemAddConstration:
  def __init__(self,model1:gp.Model,uin:dict,o:int,uout:dict):
    self.model1=model1
    self.Vars=self.addVars(model1,o)
    self.c=uout["C"]
    self.rst=self.addConstrations(self.Vars,model1,uin,o,self.c) 
  
  @staticmethod
  def addVars(model1,o) -> dict:
    # 时间
    T = 24
    # 火力发电机组数
    n_g = 5  
    socmax = 0.9#储能电量百分比
    socmin = 0.2
    return{
      #火电变量
    "p_g": model1.addMVar((n_g,T),vtype=GRB.CONTINUOUS,name=f"添加火电出力{o}"),
    "p_g_2": model1.addMVar((n_g,T),vtype=GRB.CONTINUOUS,name = f"添加火电出力的平方线性化{o}"),
    "u_g": model1.addMVar((n_g,T),vtype= GRB.BINARY,name=f"添加火电启停状态{o}"),
    "u_start": model1.addMVar((n_g,T),vtype= GRB.BINARY,name=f"添加火电启动时刻{o}"),
    "u_stop": model1.addMVar((n_g,T),vtype=GRB.BINARY,name =f"添加火电机组停止时刻{o}"),
      #水电变量：
    "p_h": model1.addMVar((1,T),vtype=GRB.CONTINUOUS,lb=0,name = f"添加水电出力{o}"),
    # 储能变量：
    "soc": model1.addMVar((1,T),vtype=GRB.CONTINUOUS,lb = socmin,ub = socmax,name = f"添加电量百分比{o}"),
    "p_ch": model1.addMVar((1,T),vtype=GRB.CONTINUOUS,lb=0,name=f"添加储能充电功率{o}"),
    "p_dis": model1.addMVar((1,T),vtype=GRB.CONTINUOUS,lb=0,name=f"添加储能放电功率{o}"),
    "u_ch": model1.addMVar((1,T),vtype=GRB.BINARY,name = f"添加储能充电状态{o}"),
    "u_dis": model1.addMVar((1,T),vtype=GRB.BINARY,name = f"添加储能放电状态{o}"),
    #风光变量：
    "p_w": model1.addMVar((1,T),vtype=GRB.CONTINUOUS,lb = 0,name=f"添加风电出力{o}"),
    "p_v": model1.addMVar((1,T),vtype=GRB.CONTINUOUS,lb = 0,name=f"添加光电出力{o}"),
     #电网变量
    "p_g_i":model1.addMVar((1, 24), vtype=GRB.CONTINUOUS,name=f"添加某时段火电{o}"),
    "pgmaxsum":model1.addMVar((1, 24), vtype=GRB.CONTINUOUS,name=f"添加某时段火电最大{o}")
    }
  @staticmethod
  def addConstrations(Vars,model1,uin,o,cadd):
    alpha = 0.9
    w_l = np.array([0.9, 1, 1.1])
    pw_f = np.array(
      [188,237,188,181,204,156,174,186,118,
                      89,77,54,52,80,82,107,144,185,163,221,215,
                      240,223,190,]
                    ).reshape(1,24)
    pv_f = np.array(
            [0,0,0,0,0,2.2000,5.5000,17.0000,28.6000,32.0000,39.0000,42.6000,42.0000,
              41.6000,40.5000,41.2000,36.5000,28.0000,16.0000,6.6000,1.1000,0,
              0,0,]
                  ).reshape(1,24)
    pload = np.array(
            [945,845,745,780,998,1095,1147,1199,1300,1397,1449,1498,1397,1297,
            1197,1048,1000,1100,1202,1375,1298,1101,900,800,]
                    )#双峰负荷曲线
    p_g_min = np.array([200, 200, 150, 120, 70])
    p_g_max = np.array([460, 400, 350, 300, 150])
    p_h_max = 280
    remp_u_d = np.array([240, 210, 150, 120, 70])  # >=pgmin
    t_on_and_off = np.array([8, 7, 6, 4, 3])
    a = 1e-5 * np.array([1.02, 1.21, 2.17, 3.42, 6.63])
    b = np.array([0.277, 0.288, 0.29, 0.292, 0.306])
    c = np.array([9.2, 8.8, 7.2, 5.2, 3.5])
    sit = np.array([25.6, 22.3, 16.2, 12.3, 4.6])
    e = np.array([0.877, 0.877, 0.877, 0.877, 0.979])
    lamda = np.array([0.94, 0.94, 0.94, 0.94, 1.03])
    # 储能模型中的常量
    capmax = 800
    p_s_max = 200
    p_s_min= 0
    socmax = 0.9#储能电量百分比
    socmin = 0.2
    theta = 0.01
    yita = 0.95
    kees = 0.009
    # 碳交易
    w = 50
    d = 100
    tao = 0.25
    #碳排放因子
    #___________
    # 循环控制变量
    # 时间
    T = 24
    # 火力发电机组数
    n_g = 5  
    '''
    约束规划
    '''
  
    #火电约束

    model1.addConstrs((Vars["p_g"][i,j]<=Vars["u_g"][i,j]*p_g_max[i] for i in range(n_g) for j in range(T)))
    model1.addConstrs((Vars["p_g"][i,j]>=Vars["u_g"][i,j]*p_g_min[i] for i in range(n_g) for j in range(T)))
    for i in range(n_g):
      for t in range(T):
        if(t==0):
          model1.addConstr(Vars["p_g"][i,t]<=remp_u_d[i])#初值设为0
        else:  
          model1.addConstr(Vars["p_g"][i,t]-Vars["p_g"][i,t-1]<=remp_u_d[i])
          model1.addConstr(Vars["p_g"][i,t-1]-Vars["p_g"][i,t]<=remp_u_d[i])
    for i in range(n_g):
      for t in range(T):
        if t==0:
          model1.addConstr(Vars["u_start"][i,t]-Vars["u_stop"][i,t] == Vars["u_g"][i,t])
        else:
          model1.addConstr(Vars["u_start"][i,t]-Vars["u_stop"][i,t] == Vars["u_g"][i,t]-Vars["u_g"][i,t-1])
        model1.addConstr(Vars["u_start"][i,t] + Vars["u_stop"][i,t] <=1)
        for k in range(t_on_and_off[i]):
          model1.addConstr(Vars["u_start"][i,t]<=Vars["u_g"][i,min(t+k,T-1)])
          model1.addConstr(Vars["u_stop"][i,t]<=1-Vars["u_g"][i,min(t+k,T-1)])
    #火电成本
    #线性化二次项
    lxpg2=np.array([np.linspace(start =0,stop=p_g_max[i],num=11) for i in range(n_g)])
    lypg2 = lxpg2**2
    for n in range(n_g):
      for t in range(T):
        model1.addGenConstrPWL(Vars["p_g"][n, t].item(), Vars["p_g_2"][n, t].item(), lxpg2[n],lypg2[n])
    c_g = 0
    c_u = 0
    for i in range(n_g):
      for t in range(T):
        c_gi = a[i]*Vars["p_g_2"][i,t]+ b[i]*Vars["p_g"][i,t]+ c[i]
        c_u =c_u+ sit[i]*(Vars["u_start"][i,t]+Vars["u_stop"][i,t])
        c_g +=c_gi
    
    #水电：
    # 约束：
    model1.addConstr(Vars["p_h"]<=p_h_max)
    # 成本：无
    
    #储能：
    soc_init = 0.5
    
  #约束：
    model1.addConstr(Vars["p_ch"]<=p_s_max*Vars["u_ch"])
    model1.addConstr(Vars["p_dis"]<=p_s_max*Vars["u_dis"])
    model1.addConstr(Vars["soc"][0,0] == soc_init*(1-theta)+yita*Vars["p_ch"][0,0]/capmax-Vars["p_dis"][0,0]/yita/capmax)
    for t in range(1,T):
      model1.addConstr(Vars["soc"][0,t] == Vars["soc"][0,t-1]*(1-theta)+Vars["p_ch"][0,t]*yita/capmax-Vars["p_dis"][0,t]/yita/capmax)
    model1.addConstr(Vars["u_ch"]+Vars["u_dis"]<=1)
    #model1.addConstr(u_ch+u_dis>=0)
    model1.addConstr(Vars["soc"][0,23]==soc_init)
    #成本：维护成本
    c_ees = kees*gp.quicksum(Vars["p_ch"][0,t].item()*yita+Vars["p_dis"][0,t].item()/yita for t in range(T))
    
    #风光电：
    F = 12#保守性调节常量
    d_p_w_max = 0.4#箱式边界系数
    d_p_v_max = 0.4
    
    u_w = uin["uw"]
    u_v = uin["uv"]
    print(f"u_w:\n{u_w}")
    print(f"u_v:\n{u_v}")
    print(f"pwf:\n{pw_f}")
    #约束：
    model1.addConstr(gp.quicksum(u_w[0,t] for t in range(T))<=F)
    model1.addConstr(gp.quicksum(u_v[0,t] for t in range(T))<=F)
    model1.addConstr(Vars["p_w"] == pw_f-u_w*d_p_w_max*pw_f)
    model1.addConstr(Vars["p_v"] == pv_f-u_v*d_p_v_max*pv_f)
    #成本:主要是弃风光惩罚
    c_wvt = 500*(pw_f-Vars["p_w"])+500*(pv_f-Vars["p_v"])
    c_wv=0
    for t in range(T):
      c_wv +=c_wvt[0,t]
    
    #负载模糊清晰化
    p_l_b = ((1 - alpha) * w_l[0] / 2+ w_l[1] / 2+ w_l[2] * alpha / 2) * pload
    print(f"plb:{p_l_b}")

    #电网：
   
    # 约束：
    for t in range(24):
        p_g_i_temp = gp.quicksum(Vars["p_g"][n, t].item() for n in range(n_g))
        model1.addConstr(Vars["p_g_i"][0,t]== p_g_i_temp)
    #model1.addConstr(p_l_b+p_ch-p_dis-p_w-p_v-p_h-p_g_i==0)
    model1.addConstr(p_l_b+Vars["p_ch"]-Vars["p_dis"]-Vars["p_w"]-Vars["p_v"]-Vars["p_h"]-Vars["p_g_i"]==0)
    
    for t in range(24):
        pgmaxsum_temp = gp.quicksum(Vars["u_g"][n, t].item() * p_g_max[n] for n in range(n_g))
        model1.addConstr(Vars["pgmaxsum"][0,t] == pgmaxsum_temp)
    #model1.addConstr(p_l_b+p_ch-p_dis-p_w-p_v-p_h-pgmaxsum<=0)
    model1.addConstr(p_l_b+Vars["p_ch"]-Vars["p_dis"]-Vars["p_w"]-Vars["p_v"]-Vars["p_h"]-Vars["pgmaxsum"]<=0)
    '''
    ------------------------------------------------------------------
    '''
    model1.addConstr( cadd >= c_g+c_ees+c_wv)
    print(f"检验C{cadd.varName}")
    model1.setObjective(cadd+c_u, GRB.MINIMIZE)
    model1.optimize()
    if model1.status != GRB.OPTIMAL:
        print("Model is infeasible. Computing IIS...")
        model1.computeIIS()
        model1.write("infeasible.ilp")
        print("IIS written to 'infeasible.ilp'")
    elif model1.status == GRB.UNBOUNDED:
        print("IIS written to 'infeasible.ilp'")
    else:
      U_start = Vars["u_start"].X
      U_stop = Vars["u_stop"].X
      U_g= Vars["u_g"].X
      U_ch = Vars["u_ch"].X
      U_dis = Vars["u_dis"].X
      LB = model1.ObjVal
      np.set_printoptions(formatter={'float_kind': '{:.2f}'.format})
      rst ={ 
              "p_g":Vars["p_g"].X,
              "p_h":Vars["p_h"].X,
              "p_ch":Vars["p_ch"].X,
              "p_dis":Vars["p_dis"].X,
              "soc":Vars["soc"].X,
              "p_w":Vars["p_w"].X,
              "p_v":Vars["p_v"].X,

              "ustart":U_start,
              "ustop":U_stop,
              "ug":U_g,
              "uch":U_ch,
              "udis":U_dis,
              "LB":LB,
              "C":cadd
      }
      return (rst)
    '''
  -------------------------------------------------------------------  
  model1.setObjective(C+c_u, GRB.MAXIMIZE)
  model1.optimize()
  if model1.status == GRB.INF_OR_UNBD:
      model1.write("infeasible.ilp")
      print("Model is infeasible. Computing IIS...")
      return 0
  elif model1.status == GRB.UNBOUNDED:
      model1.computeIIS()
      model1.write("infeasible.ilp")
      print("IIS written to 'infeasible.ilp'")
  else:
    print(model1.status)
    np.set_printoptions(formatter={'float_kind': '{:.2f}'.format})
    UBin = model1.ObjVal
    rst ={
     # "P_W":p_w.X,
      #"P_V":p_v.X,
      "p_ch":p_ch.X,
      "uw":u_w.X,
      "uv":u_v.X,
      "UBin":UBin,
    }
    print(rst)
    return(rst)
    '''




    '''
    使用类的实例化增加约束

     #储能对偶变量：
  #储能SOC约束为初始时刻、23个递推约束、初末状态相等约束总共25个
  dl_soc_max = model1.addMVar((1,24),vtype=GRB.CONTINUOUS,lb=0,name= "储能上限对偶")
  dl_soc_min = model1.addMVar((1,24),vtype=GRB.CONTINUOUS,lb=0,name = "储能下限对偶")
  dl_soc = model1.addMVar((1,25),vtype=GRB.CONTINUOUS,name= "储能soc约束对偶变量")
  dl_p_ch_max = model1.addMVar(shape=(1,24),vtype=GRB.CONTINUOUS,lb=0,name="充电功率上限对偶")
  dl_p_ch_min= model1.addMVar(shape=(1,24),vtype=GRB.CONTINUOUS,lb=0,name = "充电功率下限对偶")
  dl_p_dis_max = model1.addMVar(shape=(1,24),vtype=GRB.CONTINUOUS,lb=0,name="放电功率上限对偶")
  dl_p_dis_min= model1.addMVar(shape=(1,24),vtype=GRB.CONTINUOUS,lb=0,name = "放电功率下限对偶")
  #储能互补松弛：
  z_lch  = model1.addMVar((1,T), vtype=GRB.BINARY, name="z_lch")
  z_ldis = model1.addMVar((1,T), vtype=GRB.BINARY, name="z_ldis")
  z_au   = model1.addMVar((1,T), vtype=GRB.BINARY, name="z_au")
  z_al   = model1.addMVar((1,T), vtype=GRB.BINARY, name="z_al")
  z_bch  = model1.addMVar((1,T), vtype=GRB.BINARY, name="z_bch")
  z_bdis = model1.addMVar((1,T), vtype=GRB.BINARY, name="z_bdis")
  for t in range(T):
      # --- λ_ch * (p_ch - p_s_max * u_ch) = 0 ---
      model1.addConstr(dl_p_ch_max[0,t] <= M * z_lch[0,t])
      model1.addConstr(p_ch[0,t] - p_s_max * u_ch[0,t] >= -M * (1 - z_lch[0,t]))
      model1.addConstr(p_ch[0,t] - p_s_max * u_ch[0,t] <=  M * (1 - z_lch[0,t]))
      # --- λ_dis * (p_dis - p_s_max * u_dis) = 0 ---
      model1.addConstr(dl_p_dis_max[0,t] <= M * z_ldis[0,t])
      model1.addConstr(p_dis[0,t] - p_s_max * u_dis[0,t] >= -M * (1 - z_ldis[0,t]))
      model1.addConstr(p_dis[0,t] - p_s_max * u_dis[0,t] <=  M * (1 - z_ldis[0,t]))
      # --- α_u * (soc - soc_max) = 0 ---
      model1.addConstr(dl_soc_max[0,t] <= M * z_au[0,t])
      model1.addConstr(soc[0,t] - socmax >= -M * (1 - z_au[0,t]))
      model1.addConstr(soc[0,t] - socmax <=  M * (1 - z_au[0,t]))
      # --- α_l * (soc - soc_min) = 0 ---
      model1.addConstr(dl_soc_min[0,t] <= M * z_al[0,t])
      model1.addConstr(soc[0,t] - socmin >= -M * (1 - z_al[0,t]))
      model1.addConstr(soc[0,t] - socmin <=  M * (1 - z_al[0,t]))
      # --- β_ch * p_ch = 0 ---
      model1.addConstr(dl_p_ch_min[0,t] <= M * z_bch[0,t])
      model1.addConstr(p_ch[0,t]    <= M * (1 - z_bch[0,t]))
      # --- β_dis * p_dis = 0 ---
      model1.addConstr(dl_p_dis_min[0,t] <= M * z_bdis[0,t])
      model1.addConstr(p_dis[0,t]    <= M * (1 - z_bdis[0,t]))

  #储能平稳性
  for t in range(T):
    if t==0:
      model1.addConstr(-dl_soc[0,0]+dl_soc[0,1]*(1-theta)+dl_soc_max[0,0]-dl_soc_min[0,0]==0)
    elif t==23:
      model1.addConstr(-dl_soc[0,23]+dl_soc[0,24]+dl_soc_max[0,23]-dl_soc_min[0,23]==0)
    else:
      model1.addConstr(-dl_soc[0,t]+dl_soc[0,t+1]*(1-theta)+dl_soc_max[0,t]-dl_soc_min[0,t]==0)
  for t in range(T):
    model1.addConstr(dl_p_ch_max[0,t]-dl_p_ch_min[0,t]+kees*yita+u_ch[0,t]*yita/capmax*dl_soc[0,t]+dl_peq[0,t]+dl_prot[0,t]==0)
    model1.addConstr(dl_p_dis_max[0,t]-dl_p_dis_min[0,t]+kees/yita - u_dis[0,t]/yita/capmax*dl_soc[0,t] - dl_peq[0,t]-dl_prot[0,t]==0)
  '''